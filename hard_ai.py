...
def score(board_state):
    ...
    if len(state[b_king][1]) != 0 and len(state[w_king][1]) != 0:
        if score > 500:
            ...
            score += 100 if b_king in [11,18,81,88] else 0
        elif score < -500 :
...
def capture_search(board,a,b,token,depth,last_move, total_depth):
    global DEPTH
    string_board, state_board = (board[0],board[1])
    board_key = (string_board,str(state_board))

    moves = orderMoves((string_board, state_board), token, last_move)[0:5]
    ...
    if depth == 0 or len(moves) == 0 or game_end(board):
    ...
    for move in moves:
        ...
        if king_in_check(new_board[0],token):
            continue
        best_value = -capture_search(new_board,a,b,token*-1,depth-1, move, total_depth + 1)
        a = max(best_value,a)
        if a >= b:
            break
    ...
    if best_value <= a:
        ...
        flag = "exact"
    transposition_table[board_key] = {"value": best_value, "depth": depth, "flag": flag, "best move":best_move}
    return best_value

ply0DrawMoves = set()
ply1DrawMoves = set()
contempt_factor = 650
def pvs(board, depth, token, a, b, f, first_depth,prev_move, total_depth):
    string_board, state_board = (board[0],board[1])
    board_key = (string_board,str(state_board))
    ...
    if depth == 0 or game_end(board):
        returning_value = capture_search(board,a,b,token,1,prev_move, total_depth + 1)#token*score(board)#
        if returning_value <= a:
            flag = "lowerbound"
        elif returning_value >= b:
            flag = "upperbound"
    ...
    for moves in children:
        ...
        if f == "ab":
            prev_val = value
            value = max(value,-pvs(boards,depth-1,-token,-b,-a,"ab",first_depth,moves, total_depth + 1))
            if value != prev_val:
                best_move = moves
        else:
            prev_val = value
            value = max(value,-pvs(boards,depth-1,-token,-a-1,-a,"zw",first_depth,moves, total_depth + 1))
            if value != prev_val:
                best_move = moves
            if a < value < b:
                prev_val = value
...
def find_next_move(board,token,depth):
    ...
    bestMove = firstMove
    bestEval = -pvs((k:=make_move(board,firstMove)),depth-1,-token_key, -beta,-alpha,"ab",depth-1,firstMove,1)
    ...
    for move in allMoves:
        ...
        if new_board[0] in ply0DrawMoves:
            ...
            evalMove = -pvs(new_board,depth-1,-token_key, -alpha-1,-alpha,"zw",depth-1,move,1)
            if alpha < evalMove < beta:
                evalMove = -pvs(new_board,depth-1,-token_key, -beta,-alpha,"ab",depth-1,move,1)
...
#          [".", ".", ".", ".", ".", ".", ".", "."],  #7
#          ["k", ".", "K", ".", ".", ".", ".", "."]]  #8
# str_board = list_to_board(board)#conver_fen_to_board("rn1qkbnr/pp2pppp/2p5/5b2/2p5/P3P3/1P1PBPPP/RNBQK1NR")
...
board_rep = (board_state[0],board_state[1].copy())

ply0_moves = possible_moves_gui(board, player)
for move in ply0_moves:
    ...
    ply1_moves = possible_moves_gui(new_new_board, player*-1)
...
for i in range(10):
    print(find_next_move(board_rep, player, depth))
    depth += 1
